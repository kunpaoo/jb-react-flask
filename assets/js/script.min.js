...
eval(config),
  (function () {
    ...
    var t = document.querySelector(".sidebar"),
      e = document.querySelectorAll("#sidebarToggle, #sidebarToggleTop");
    if (t) {
      t.querySelector(".collapse");
      var n = [].slice
        .call(document.querySelectorAll(".sidebar .collapse"))
        .map(function (t) {
          return new bootstrap.Collapse(t, { toggle: !1 });
        });
      for (var s of e)
      ...
      window.addEventListener("resize", function () {
        if (
          ...
          for (var t of n) t.hide();
      });
    }
    var a = document.querySelector("body.fixed-nav .sidebar");
    a &&
      a.on("mousewheel DOMMouseScroll wheel", function (t) {
        if (
          Math.max(
            document.documentElement.clientWidth || 0,
            window.innerWidth || 0
          ...
          (this.scrollTop += 30 * (n < 0 ? 1 : -1)), t.preventDefault();
        }
      });
